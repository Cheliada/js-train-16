...
function compareArrays(arr1, arr2) {
  ...
  console.assert(
      JSON.stringify(myArr1) === JSON.stringify(myArr2),
      "Масиви не однакові!"
  );
...
function displayGroupedInfo(students) {
  for (const element of students) {
console.group(`Студент: ${element.name}`);
console.info(`Вік: ${element.age}`);
console.info(`Предмет: ${element.subject}`);
console.groupEnd();
  }
...
function countVowelsAndConsonants(word) {
  ...
  for (const element of word) {
      if (myString.includes(element.toLowerCase())) {
          console.log("Кількість голосних: ", ++numA);
      } else {
          console.log("Кількість приголосних: ", ++numB);
      }
  }
